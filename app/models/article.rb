class Article < ApplicationRecord
  ...
  scope :articles_per_category, lambda { |category_id|
    includes(:author).where('category_id = ? and status = ?', category_id, 'published')
  }

  scope :category_all_article, lambda {
    Category.includes(:articles).where(articles: { status: 'published' }).order(priority: :desc)
  }

  scope :featured_article, lambda {
    all_published_articles.joins(:votes).group(:id).count.max_by { |_k, v| v }
  }
  ...
  scope :user_pub_articles, lambda { |current_user|
    where('author_id = ? and status = ?', current_user, 'published')
  }
  ...
  scope :suggested_article, lambda { |tag, category_id|
    all_published_articles.joins(:taggings).where('tag_id = ? or category_id = ? ', tag, category_id)
  }
  ...
  def self.search_article(search)
    ...
    arr = []
    all_published_articles.unscope(:includes).each do |article|
